...
public class GetdownClient implements ClientModInitializer {
    ...
    public static final SimpleOption<Boolean> CRAWL_TOGGLED = new SimpleOption<>(
        KEY,
    ...
    public void onInitializeClient() {
        ClientTickEvents.END_CLIENT_TICK.register(client -> {
            ...
            if (CrawlState.setCrawling(player, crawlPressed)) ClientPlayNetworking.send(new CrawlState(crawlPressed));
        });
    }
    ...
    public static SimpleOption<?>[] injectGetDownOption(GameOptionsAccessor gameOptions, SimpleOption<?>[] options) {
        ...
        list.add(sprintToggleIndex + 1, CRAWL_TOGGLED);
